...
with open('5-seed-fertilizer-input.txt') as input:
    ...
    for i, raw_seed in enumerate(raw_seeds):
        if i % 2 == 0:
            seeds.append((raw_seed, raw_seeds[i+1]))
    ...
    for (seed, seed_range) in seeds:
        seed_int = int(seed)
        max_seed = seed_int + int(seed_range) - 1

        current_values = [int(seed)]
            ...
            for current_value in current_values:
                for i, almanac_value in enumerate(current_almanac):
                    if int(almanac_value[1]) + int(almanac_value[2]) < current_value:
                        if i == len(current_almanac) - 1:
                            new_values.append(current_value)
                        continue
                    if int(almanac_value[1]) < current_value:
                        diff = current_value - int(current_almanac[i][1])
                        new_values.append(int(current_almanac[i][0]) + diff)             
                    if i < len(current_almanac) - 1 and max_seed > int(current_almanac[i+1][1]) + int(current_almanac[i+1][2]):
                        new_values.append(int(current_almanac[i+1][0]))
                    break
            if almanac_type == Almanac.HUMIDITY_TO_LOCATION:
                locations.append(min(new_values))
