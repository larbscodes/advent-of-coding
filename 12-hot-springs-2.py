...
with open('12-hot-springs-input.txt') as input:
    ...
    arrangements = 0
    for line in input:
        [raw_springs, contiguous_damaged_springs_input] = line.strip().split(' ')
        springs = ((raw_springs + '?') * 5)[:-1]
        contiguous_damaged_springs = list(map(lambda x: int(x), contiguous_damaged_springs_input.split(','))) * 5
